...
A.KEYS = {
    ...
    -- GENERATED by scripts/gen_nameplate_bake.py: the slices above pre-rendered for a 148x16 bar.
    barBgFull = { A.SHEET.bake, 2 / 512, 158 / 512, 2 / 128, 27 / 128, 156, 25, 12, 7, 12, 10 },
    selectedFull = { A.SHEET.bake, 2 / 512, 158 / 512, 30 / 128, 54 / 128, 156, 24, 8, 8, 8, 8 },
    deselectedFull = { A.SHEET.bake, 2 / 512, 152 / 512, 57 / 128, 75 / 128, 150, 18, 5, 5, 5, 5 },
    barFillTall = { A.SHEET.bake, 2 / 512, 126 / 512, 78 / 128, 94 / 128, 124, 16, 4, 5, 4, 4 },
    barBgTall = { A.SHEET.bake, 260 / 512, 392 / 512, 2 / 128, 27 / 128, 132, 25, 12, 7, 12, 10 },
    selectedTall = { A.SHEET.bake, 260 / 512, 476 / 512, 30 / 128, 54 / 128, 216, 24, 8, 8, 8, 8 },
    deselectedTall = { A.SHEET.bake, 260 / 512, 470 / 512, 57 / 128, 75 / 128, 210, 18, 5, 5, 5, 5 },
}

-- A bake replaces its key only at the rect it was rendered for (Full: width and height, Tall: height).
local BAKES = {
    barBg = { { "barBgFull", true }, { "barBgTall" } },
    selected = { { "selectedFull", true }, { "selectedTall" } },
    deselected = { { "deselectedFull", true }, { "deselectedTall" } },
    barFill = { { "barFillTall" } },
}

local function ResolveBake(key, rectW, rectH)
    local list = BAKES[key]
    if not list then
        return key
    end
    for i = 1, #list do
        local k = A.KEYS[list[i][1]]
        if abs(k[7] - rectH) < 0.01 and (not list[i][2] or abs(k[6] - rectW) < 0.01) then
            return list[i][1]
        end
    end
    return key
...
function A.LayoutSlice(slice, key, anchor, w, h, padL, padT, padR, padB)
    if not slice or not A.KEYS[key] or not anchor then
        return false
    end
    ...
    key = ResolveBake(key, rectW, rectH)
    local k = A.KEYS[key]
    ...
    -- An axis whose caps scale with the rect is a plain stretch: one span draws it identically.
    local rowsMerged = abs(capT / rectH - mT / nativeH) < 1e-4 and abs(capB / rectH - mB / nativeH) < 1e-4
    local colsMerged = abs(capL / rectW - mL / nativeW) < 1e-4 and abs(capR / rectW - mR / nativeW) < 1e-4
    if rowsMerged then
        rowH[1], rowH[3] = 0, 0
        rowY[2], rowH[2], rowT[2], rowB[2] = padT, rectH, t, b
    end
    if colsMerged then
        colW[1], colW[3] = 0, 0
        colX[2], colW[2], colL[2], colR[2] = -padL, rectW, l, r
    end
    -- The UV cache is keyed on the key alone; a merged span samples a wider rect of the same key.
    local merge = (rowsMerged and 1 or 0) + (colsMerged and 2 or 0)
    if slice.merge ~= merge then
        slice.merge = merge
        for i = 1, 9 do
            keyed[i] = nil
        end
